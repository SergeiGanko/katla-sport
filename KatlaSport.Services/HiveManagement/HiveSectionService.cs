    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        public async Task<HiveSection> CreateHiveSectionAsync(UpdateHiveSectionRequest createRequest)
        {
            var dbSections = await _context.Sections
                                 .Where(hs => hs.Code == createRequest.Code)
                                 .ToArrayAsync()
                                 .ConfigureAwait(false);
            ...
            var dbHives = await _context.Hives
                              .Where(h => h.Id == createRequest.HiveId)
                              .ToArrayAsync()
                              .ConfigureAwait(false);
        ...
        {
            var dbSections = await _context.Sections
                                 .Where(hs => hs.Code == updateRequest.Code && hs.Id != hiveSectionId)
                                 .ToArrayAsync()
                                 .ConfigureAwait(false);
            ...
            dbSections = await _context.Sections
                             .Where(hs => hs.Id == hiveSectionId)
                             .ToArrayAsync()
                             .ConfigureAwait(false);
