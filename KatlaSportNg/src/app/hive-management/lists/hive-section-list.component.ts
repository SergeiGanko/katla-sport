...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    var hiveSection = this.hiveSections.find(hs => hiveSectionId == hs.id);
    this.hiveSectionService.setHiveSectionStatus(hiveSection.id, true).subscribe(s => hiveSection.isDelited = true)
  }

  obUndelete(hiveSectionId: number) {
    var hiveSection = this.hiveSections.find(hs => hiveSectionId == hs.id);
    this.hiveSectionService.setHiveSectionStatus(hiveSection.id, false).subscribe(s => hiveSection.isDelited = false)
  }
}
